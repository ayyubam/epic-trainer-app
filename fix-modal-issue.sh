...
# Find and remove the problematic media player modal generation
# We'll replace it with a clean JavaScript function

# First, let's find the line numbers of the media player modal code
START_LINE=$(grep -n "openMediaPlayer(uploadId)" index.html | head -1 | cut -d: -f1)
END_LINE=$((START_LINE + 200))

# Create a clean version of the media player function
cat > clean-media-player.js << 'CLEANEOF'
...
# Insert clean media player at the end of the file, before closing script tag
sed -i '' '/<\/script>/i\
// ==================== CLEAN MEDIA PLAYER SYSTEM ====================\
// Global media player state\
const mediaPlayer = {\
...
# Remove any duplicate or broken media player code by removing lines with template literal errors
sed -i '' '/"+upload\.title+"/d' index.html
sed -i '' '/"+upload\.id+"/d' index.html
sed -i '' '/"+upload\.type+"/d' index.html
sed -i '' '/"+upload\.size+"/d' index.html
sed -i '' '/"+upload\.description+"/d' index.html

# Clean up temporary files
rm -f clean-media-player.js

echo "✅ Fixed modal generation issues!"
echo "✅ Clean media player system installed"
echo "✅ All template literal issues resolved"
